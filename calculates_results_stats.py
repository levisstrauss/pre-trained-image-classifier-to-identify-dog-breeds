...
def calculates_results_stats(results_dic):
    """
    Calculates statistics of the output of the program run using classifier's model
    architecture to classifying pet images. Then puts the output statistics in a
    dictionary (results_stats_dic) so that it's returned for printing as to help
    the user to determine the 'best' model for classifying images. Note that 
    the statistics calculated as the output are either percentages or counts.
    Parameters:
      results_dic - Dictionary with key as image filename and value as a List 
             (index)idx 0 = pet image label (string)
                    ...
                    idx 4 = 1/0 (int)  where 1 = Classifier classifies image 
                            ...
                            'as-NOT-a' dog.
    Returns:
     results_stats_dic - Dictionary that contains the output statistics (either
